// Public original UI + server-verified studio session adapter.
// Studio pages and APIs require an HttpOnly session issued after server-side password verification.
import { listPage, articlePage, shell } from './ssul-render.mjs';
...
const STUDIO_COOKIE='__Host-ssulpan_studio';
...
function hashtagText(value){return Array.isArray(value)?value.filter(tag=>typeof tag==='string'&&tag.trim()).map(tag=>tag.trim()).join(' '):String(value||'')}
function siteContent(p){const value=originalContent(p||{}),raw=dbId(value.id),fade=Number(value.fadeHeight??value.fade_height??180);return{id:/^[1-6]$/.test(raw)?raw.padStart(3,'0'):raw,title:String(value.title||''),category:String(value.category||'일상'),teaser:String(value.teaser||''),beforeContent:String(value.beforeContent||''),afterContent:String(value.afterContent||''),hook:String(value.hook||''),coverDetail:String(value.coverDetail||''),caption:String(value.caption||''),hashtags:hashtagText(value.hashtags),fadeHeight:Math.max(80,Math.min(300,Number.isFinite(fade)?fade:180)),gateLine:String(value.gateLine||''),views:Number(value.views??value.view_count??0)}}
async function studioCall(payload,timeout=170000){const response=await fetch(STUDIO_API,{method:'POST',headers:{apikey:PUBLIC_KEY,'Content-Type':'application/json'},body:JSON.stringify(payload),signal:AbortSignal.timeout(timeout)});let body;try{body=await response.json()}catch{throw Object.assign(new Error('제작실 서버 응답을 읽지 못했어요.'),{status:502})}if(!response.ok||!body.ok)throw Object.assign(new Error(body.error||'제작실 요청을 처리하지 못했어요.'),{status:response.status,diagnostics:body.diagnostics});return body}
...
function cookieValue(request,name){const found=(request.headers.get('Cookie')||'').split(';').map(value=>value.trim()).find(value=>value.startsWith(name+'='));if(!found)return'';try{return decodeURIComponent(found.slice(name.length+1))}catch{return''}}
function sessionCookie(token,expiresAt){const seconds=Math.max(0,Math.min(43200,Math.floor((Number(expiresAt)-Date.now())/1000)));return `${STUDIO_COOKIE}=${encodeURIComponent(token)}; Path=/; Max-Age=${seconds}; HttpOnly; Secure; SameSite=Strict`}
function clearSessionCookie(){return `${STUDIO_COOKIE}=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Strict`}
async function studioAuthorized(request){const token=cookieValue(request,STUDIO_COOKIE);if(!token)return false;try{return (await studioCall({action:'studio_access_verify',token},15000)).authenticated===true}catch{return false}}
function protectedStudioPage(path){return ['/studio','/studio/','/studio.html','/studio/preview/','/instagram','/instagram/','/instagram.html','/manuscript','/manuscript/','/manuscript.html'].includes(path)}
async function studioApi(request,path){const method=request.method,u=new URL(request.url);
  const call=(payload,timeout)=>studioCall({...payload,studioSession:cookieValue(request,STUDIO_COOKIE)},timeout);
  ...
  if(path==='/api/writing-prompt'){if(method==='GET')return json(await call({action:'writing_prompt_get'}));if(method==='PUT'){const b=await requestBody(request);return json(await call({action:'writing_prompt_save',prompt:b.prompt,revision:b.revision}));}}
  ...
  if(path==='/api/jobs'&&method==='POST'){const b=await requestBody(request),encoder=new TextEncoder();const stream=new ReadableStream({start(controller){let open=true;const send=value=>{if(open)try{controller.enqueue(encoder.encode(JSON.stringify(value)+'\n'))}catch{open=false}};send({type:'started',jobId:b.jobId});let progressMessage='클로드가 요청한 내용을 작성하고 있어요.';send({type:'progress',message:progressMessage});const timer=setInterval(()=>send({type:'progress',message:progressMessage}),15000);(async()=>{try{const out=await call({...b,action:'job_run',jobAction:b.action});progressMessage='Claude 작업을 완료했어요. 결과를 확인해 주세요.';send({type:'progress',message:progressMessage});send({type:'done',jobId:out.id,result:out.result,usage:out.usage,model:out.model,calls:out.calls,promptUsage:out.promptUsage,promptRevision:out.promptRevision,baseData:out.baseData,target:out.target,diagnostics:out.diagnostics})}catch(error){send({type:'failed',jobId:b.jobId,message:error?.message||'작업 연결이 끊겼어요. 작업 기록에서 상태를 확인해 주세요.',diagnostics:error?.diagnostics||{jobId:b.jobId,stage:'worker_transport',error:{code:'WORKER_CONNECTION',message:error?.message||'서버 연결 종료'},note:'연결 종료만으로 서버 작업 실패가 확정되지는 않습니다. 작업 기록을 확인하세요.'}})}finally{clearInterval(timer);if(open)try{controller.close()}catch{}}})()}});return new Response(stream,{headers:{'Content-Type':'application/x-ndjson; charset=utf-8','Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}})}
  if(path==='/api/source'&&method==='POST'){const b=await requestBody(request);return json(await call({action:'source_read',url:b.url}));}
  if(path==='/api/images'&&method==='POST'){const mime=request.headers.get('Content-Type')||'',ab=await request.arrayBuffer();if(ab.byteLength>4*1024*1024)return json({error:'이미지는 4MB 이하로 올려 주세요.'},413);return json(await call({action:'image_upload',name:u.searchParams.get('name')||'이미지',mime,base64:arrayB64(ab)}));}
  const image=path.match(/^\/api\/images\/([\w-]+)$/);if(image&&method==='GET'){const out=await call({action:'image_url',id:image[1]});return Response.redirect(out.url,302)}
...
export default{async fetch(request,env){const url=new URL(request.url),path=url.pathname;
  try{
    ...
    if(path==='/stories.json'&&request.method==='GET'){try{const result=await data(API+'?limit=100');return json((result.posts||[]).map(siteContent))}catch{return json({error:'게시글 서버 연결 실패'},502)}}
    ...
    try{const result=await data(API+'?limit=100'),posts=(result.posts||[]).map(siteContent);if(home){const page=path==='/'?1:Number(path.split('/')[2]),out=listPage(posts,page,Object.fromEntries(url.searchParams));return html(request.method==='HEAD'?'':out||shell('글을 찾을 수 없어요','<main id="main" class="wrap empty-page"><h1>페이지를 찾을 수 없어요.</h1><a href="/">전체 글</a></main>'),out?200:404)}const id=dbId(decodeURIComponent(article[1]));let post=posts.find(p=>dbId(p.id)===id);if(!post){try{post=siteContent((await data(API+'?id='+encodeURIComponent(id))).post)}catch{return html(shell('글을 찾을 수 없어요','<main id="main" class="wrap empty-page"><h1>글을 찾을 수 없어요.</h1><a href="/">전체 글</a></main>'),404)}}return html(request.method==='HEAD'?'':articlePage(post,posts,false))}catch(error){console.error('ssul public backend:',error.message);return html(shell('연결 확인','<main id="main" class="wrap empty-page"><h1>게시글 서버에 연결하지 못했어요.</h1><p>잠시 후 새로고침해 주세요.</p><a href="/">다시 시도</a></main>'),503)}
  }catch(error){console.error('ssul worker:',error?.message||error);const status=Number(error?.status)||500;return path.startsWith('/api/')?json({error:error?.message||'요청을 처리하지 못했어요.',diagnostics:error?.diagnostics},status):html('<!doctype html><meta charset="utf-8"><title>오류</title><p>요청을 처리하지 못했어요.</p>',status)}
}};
