...
export function promptFor(action,d,target,instruction,writingPrompt=''){
  ...
  const bodyPolicy='원문의 사건·동기·중요 대사·감정 변화·결말이 전달되었는지 확인하고 모두 전달되면 끝낸다. 같은 정보를 다른 말로 반복하거나 새 사건으로 연장하지 않는다. 원문이 미해결이면 억지 결말을 추가하지 않는다. 본문 편집의 우선 기준: 원문의 핵심 사건, 인물 관계, 중요한 대사의 의미, 반전과 결말, 재미를 만드는 구체적인 장면을 살린다. 원문을 읽기 좋고 몰입감 있게 재구성하되 사용자가 명시적으로 요청하지 않은 핵심 사건이나 반전을 새로 만들지 않는다. 고정 목표 글자 수, 최소 글자 수, 글자 수 허용 오차, 원문 대비 증감률, 광고 전후 분량 비율을 적용하지 않는다. 짧은 원문을 억지로 늘리거나 긴 원문의 중요한 장면을 분량 때문에 빼지 않는다. 중복과 불필요한 설명은 다듬되 숫자를 맞추기 위해 요약하거나 부풀리지 않는다. 광고 전후는 사건 흐름상 궁금증이 생기는 자연스러운 전환점에서 나눈다. 저장된 프롬프트에 예전 분량·비율 규칙이 있더라도 이 본문 편집 기준을 우선한다. 표지와 소개 문구의 화면 규격은 별도다. 서버가 확인해 전달한 options.lengthRequest와 notes·rewriteInstruction의 직접적인 축약·확장 요청만 편집 방향으로 반영한다. 명시적인 요청도 결과 글자 수 오차 검사나 자동 재생성 조건으로 사용하지 않는다. 부분 수정의 요청은 허용된 범위 안에서만 반영한다.';
